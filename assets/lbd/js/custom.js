$(document).ready(function () {
    ...
    $(document).on('click', '.invoke-chat', function(e){
        e.preventDefault()
        $('.floating-module').trigger('click');
    })
    // Trigger floating box from message button end
    //Floating Module
    ...
    $('.tab-panel').mCustomScrollbar({
        ...
        autoHideScrollbar: true,
        advanced:{
            // updateOnSelectorChange: "div.import-teamlist-container",
            updateOnContentResize: true
        },
        mouseWheel: {
            // preventDefault: true,
            // disableOver: ["ul", "select", "option", "keygen", "datalist", "textarea"]
        }
    });

    /* $('.tab-panel-container .dropdown-menu.open .dropdown-menu.inner').mCustomScrollbar({
        ...
        live: "on",
        liveSelector: ".dropdown-menu.inner"
    }); */

    /* $('div').delegate('.tab-panel-container .dropdown-menu.inner .mCSB_scrollTools_vertical', 'click', function(e){
        e.preventDefault();
    ...
    // Quote tags
    // appenTo option that results in an empty jQuery collection
    var availableTags = [
        "ActionScript",
        "AppleScript",
        "Asp",
      ...
      /* $( "input#quote-tags" ).autocomplete({
            source: availableTags,
            appendTo: $('.quote-tags-suggesstion'),
            position: { my: "left bottom", at: "left top"}
      }); */
    $('input#quote-tags').tagedit({
        autocompleteURL: availableTags,
        autocompleteOptions: {
            // source: availableTags,
            appendTo: $('.quote-tags-suggesstion'),
            position: { my: "left bottom", at: "left top"}
    ...
    //Settings popup
    $('.btn-mission-overview-reassign').click(function () {
        $('.overlay-main-container, .settings-wrap').fadeIn(function () {
            $('.overlay-main-container, .mission-overview-reassign').css({
                display: 'block'
            });
    ...
    /* $('.bs-searchbox').siblings('.dropdown-menu').addClass('search-box');
    $('.bs-actionsbox').siblings('.dropdown-menu').addClass('actions-box'); */
    ...
    /* Update mCustomScrollbar after tabs shown */
    $('.tab-step-content a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
        /* e.target // newly activated tab
        e.relatedTarget // previous active tab */
        $(".tab-panel-container .tab-panel").mCustomScrollbar("update");
    })

    $(document).on('mouseleave', '.tab-panel-container .dropdown-menu, .tab-panel-container .chosen-drop, .tags_suggestions ul.ui-autocomplete', function (e) {
        console.log('Mouse leave...')
        e.stopPropagation();
        e.preventDefault();
        e.stopImmediatePropagation();
        var scrollContainerHeight = $(this).parents('.mCSB_container').outerHeight(),
            scrollBoxHeight = $(this).parents('.mCustomScrollBox').outerHeight();
        if(scrollBoxHeight > scrollContainerHeight){
            // $(this).parents('.mCSB_container').css({"height":"auto"})
            $(this).parents('.mCSB_container').css({"height":""})
            $(".tab-panel-container .tab-panel").mCustomScrollbar("update");
        };
        $(".tab-panel-container .tab-panel").mCustomScrollbar("update");
    });
    $(function () {
        ...
        /* var toolbox = $('.tab-panel-container .chosen-drop, .tab-panel-container .dropdown-menu'),
        ...
        $('.tab-panel-container .chosen-drop, .tab-panel-container .dropdown-menu, .tags_suggestions ul.ui-autocomplete').on('mousewheel touchmove', function(e){
            console.log('Mousewheel...')
            // $(this).parents('.tab-panel').off('mousewheel');
            ...
            $(".tab-panel-container .tab-panel").mCustomScrollbar("disable");
            // $(".tab-panel-container .tab-panel.mCS_disabled .mCSB_container").css({"height":"auto"})
    ...
    $(document).on("shown.bs.select", ".bootstrap-select", function (e) {
        ...
        var $ul = $(this).children(".dropdown-menu"),
            ...
            $button = $(this).children(".dropdown-toggle"),
            ulOffset = $ul.offset(),
            $searchBox = $(this).find('.bs-searchbox').length,
            $actionsBox = $(this).find('.bs-actionsbox').length,
            // how much space would be left on the top if the dropdown opened that direction
            ...
            // switch to dropup only if there is no space at the bottom AND there is space at the top, or there isn't either but it would be still better fit
        // console.log($searchBox, $actionsBox);
        /* if($searchBox >= 1) {
            $($ul).children('.dropdown-menu').addClass('search-box')
        }
        if($actionsBox >= 1) {
            $($ul).children('.dropdown-menu').addClass('actions-box')
        } */
        if (spaceDown < 0 && (spaceUp >= 0 || spaceUp > spaceDown)) {
            $(this).addClass("dropup");
            $('.dropup .dropdown-menu.open').css({
                'bottom': ($button.outerHeight() - 5),
                'margin-bottom': '-5px'
            });
        }
        $('.bootstrap-select.open .dropdown-menu.open').css({
            'bottom': '',
            'margin-bottom': ''
        });
        /* $(function(){
            if($(this).find('.dropdown-menu.inner').hasScrollBar()){
                $('.dropdown-menu.inner').css({'padding-bottom':'20px'})
            }
        })   */
    }).on("hidden.bs.select", ".bootstrap-select", function () {
        // always reset after close
        $(this).removeClass("dropup");
        $(this).find('.dropdown-menu.open').css({
            'bottom': '',
            'margin-bottom': ''
        });
        /* $(this).find('.dropdown-menu.inner').css({'padding-bottom':''}) */
        // $('.tab-panel-container .tab-panel').mCustomScrollbar('update');
    });
    ...
    $(document).on("chosen:showing_dropdown", "select.chosen-select, select.chosen-default", function (e, t) {
        ...
        var chosenDrop = $(this).next().find('.chosen-drop'),
            ...
            $lastli = $($ul).find('li:last-child'),
            $button = $(this).next().find(".chosen-choices, .chosen-single"),
            ulOffset = $ul.offset();
        // how much space would be left on the top if the dropdown opened that direction
        var spaceUp = (ulOffset.top - $button.height() - $ul.height()) - $(window).scrollTop();
        // how much space is left at the bottom
    ...
    /* Mission info show in the list */
    $('a.show-info').click(function(){
        // $(this).parents('.card').parent('li').siblings().find('.mission-info-container').slideUp(5);
        $(this).parents('.card').siblings('.mission-info-container').slideToggle(5);
    });
    /* $('a.show-info').click(function(){
        $(this).parents('li').siblings().find('.mission-info-container').removeClass('show-info-container');
        $(this).parents('li').find('.mission-info-container').addClass('show-info-container');
    }); */
